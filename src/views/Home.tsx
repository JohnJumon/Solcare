...
import { API_BASE_URL, PROPOSAL_SEED, USDC_DECIMALS, getDerivedAccount, now } from '../utils';
...
const Home = () => {
    ...
    const fetchDonatedCampaign = async () => {
        if (connected && publicKey) {
            ...
            await Promise.all(donors.map(async (e) => {
                try {
                    const campaignData = await smartContract.account.campaign.fetch(e.account.campaign)

                    const response = await axios.get(API_BASE_URL + '/v1/campaign/' + e.account.campaign);
                    ...
                    proposalPubkeys.push(getDerivedAccount(
            ...
            const proposals = await smartContract.account.proposal.fetchMultiple(proposalPubkeys)
            await Promise.all(proposals.map(async (e) => {
                if (e !== null) {
                    const acc = (e as any);
                    if (now() < acc.createdAt.toNumber() + acc.duration.toNumber()) {
                        try {
                            const response = await axios.get(API_BASE_URL + '/v1/campaign/' + acc.campaign);
                            const responseData = response.data.data;
